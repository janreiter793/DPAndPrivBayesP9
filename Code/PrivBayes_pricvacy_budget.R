...
# Parameters
eps   <- seq(0.1, 100, length.out = 1000)
beta <- 0.5
iterations <- 500
...
threads <- function(N) {
  ...
  for(e in eps) {
    ...
    #    e, "and beta =", beta, "\n")
    iter_star <- iterations / 10
    for(iter in (iterations * (N - 1) / 10 + 1):(iterations * N / 10)) {
      #cat(iter, "/", iterations, "\n")
      privbayes_res <- try({  
        PrivBayes(epsilon = e)
      }, silent = TRUE)
    ...
    res[[as.character(e)]] <- z / iter_star
  }
...
plotPowers <- function() {
  attr_1_and_2 <- eps %>% length %>% numeric
  attr_1_and_3 <- eps %>% length %>% numeric
  attr_1_and_4 <- eps %>% length %>% numeric
  attr_1_and_5 <- eps %>% length %>% numeric
  for(i in 1:length(eps)) {
    privacy <- as.character(eps[i])
    attr_1_and_2[i] <- res[[privacy]][2,1]
    attr_1_and_3[i] <- res[[privacy]][3,1] 
    attr_1_and_4[i] <- res[[privacy]][4,1]
    attr_1_and_5[i] <- res[[privacy]][5,1]
  }
  matplot(x = eps, cbind(1 - attr_1_and_2,
                           1 - attr_1_and_3,
                           1 - attr_1_and_4,
                           1 - attr_1_and_5), 
          type = "l", lty = 1,
          ylab = "Power", xlab = "epsilon",
          main = paste0("Estimated power of Chi-squared test on PrivBayes data (beta = ",
                        beta, ")"),
          panel.first = grid(),
